...
// Safe storage that won't crash on SSR
    ...
    (set, get) => ({
      ...
      joinRoom: (code, memberName, avatar) => {
        ...
        set((state) => ({
          ...
          joinedRooms: [
            ...(state.joinedRooms ?? []).filter(jr => jr.roomId !== room.id),
      ...
      getRoomByCode: (code) => get().rooms.find((r) => r.code === code.toUpperCase()),
      ...
      sendMessage: (content) => {
        ...
        set((s) => ({
          rooms: s.rooms.map((r) =>
            r.id === currentRoomId ? { ...r, messages: [...r.messages, message] } : r
          ),
        }));
      },
        ...
        set((s) => ({
          rooms: s.rooms.map((r) => {
            ...
            if (existing) {
              ...
              return {
                ...r,
                dateVotes: r.dateVotes
                  .map((v) => v.date === date
                    ? { ...v, memberIds: hasVoted ? v.memberIds.filter(id => id !== currentMemberId) : [...v.memberIds, currentMemberId] }
                    : v
                  )
                  .filter((v) => v.memberIds.length > 0),
              };
            }
            return { ...r, dateVotes: [...r.dateVotes, { date, memberIds: [currentMemberId] }] };
          }),
        }));
      },
      ...
      addLocation: (location) => {
        ...
        set((s) => ({
          rooms: s.rooms.map((r) =>
            r.id === currentRoomId ? { ...r, locations: [...r.locations, location] } : r
          ),
        }));
      },
      ...
      removeLocation: (roomId, locationName) =>
        set((s) => ({
          rooms: s.rooms.map((r) =>
            r.id === roomId ? { ...r, locations: r.locations.filter((l) => l.name !== locationName) } : r
          ),
        })),
        ...
        set((s) => ({
          rooms: s.rooms.map((r) =>
            r.id === currentRoomId
              ? {
                  ...r,
                  members: r.members.map((m) => m.id === currentMemberId ? { ...m, avatar } : m),
                  messages: r.messages.map((msg) => msg.memberId === currentMemberId ? { ...msg, memberAvatar: avatar } : msg),
                }
              : r
          ),
        }));
