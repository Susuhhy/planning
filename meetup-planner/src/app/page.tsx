...
export default function Home() {
  ...
  const { createRoom, joinRoom, rooms, joinedRooms, switchRoom } = useStore();
  ...
  const handleCreate = () => {
    ...
    setError('');
    const { room } = createRoom(roomName.trim(), maxMembers, roomProfile || undefined);
    const result = joinRoom(room.code, memberName.trim(), memberAvatar || undefined);
  ...
  const handleJoin = () => {
    ...
    if (!memberName.trim()) { setError('닉네임을 입력해주세요'); return; }
    setError('');
    const result = joinRoom(joinCode.trim().toUpperCase(), memberName.trim(), memberAvatar || undefined);
  ...
  const myRooms = joinedRooms
    .map(jr => {
      ...
      return room && member ? { room, member, memberId: jr.memberId } : null;
    })
    .filter(Boolean) as { room: (typeof rooms)[0]; member: { id: string; name: string; avatar?: string; joinedAt: string }; memberId: string }[];
    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 440 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <h1 style={{ fontSize: 42, color: 'var(--accent)', marginBottom: 4 }}>Planit</h1>
          <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>친구들과 쉽게 약속 잡기</p>
          <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>created by @shuuuhn</p>
        </div>

        {/* My rooms */}
        {myRooms.length > 0 && (
          <div style={{ marginBottom: 20 }}>
            <h3 style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 10 }}>내 방 목록</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {myRooms.map(({ room, member }) => {
                ...
                return (
                  <button
                    key={room.id}
                    onClick={() => handleEnterRoom(room.id, room.code)}
                    style={{
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{room.name}</div>
        ...
        <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                <div onClick={() => roomImgRef.current?.click()} style={{
                  ...
                  {roomProfile
                    ? <img src={roomProfile} alt="방 프로필" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    : '📸'}
                </div>
                <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap' }}>
                  {[2, 3, 4, 5, 6, 8, 10].map((n) => (
                    <button key={n} onClick={() => setMaxMembers(n)} style={{
                      flex: 1, minWidth: 36, padding: '6px 0', borderRadius: 'var(--radius-sm)',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div onClick={() => avatarRef.current?.click()} style={{
                  ...
                  background: 'var(--teal-light)', border: '2px dashed var(--border-strong)',
                  cursor: 'pointer', overflow: 'hidden', flexShrink: 0,
                  display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22,
                }}>
                  {memberAvatar
                    ? <img src={memberAvatar} alt="내 프로필" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    : '😊'}
                </div>
                <div style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                ...
                <input className="input-field"
                  style={{ marginTop: 4, textTransform: 'uppercase', letterSpacing: 4, fontSize: 18, fontWeight: 700, textAlign: 'center' }}
          ...
          {error && (
            <div style={{
              ...
              background: '#fdf0ec', border: '1px solid #f5bfb0',
