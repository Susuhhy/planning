...
export default function RoomPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  // Find room by URL code (primary) and session (for member lookup)
  const room: Room | undefined = store.getRoomByCode(code);
  const currentMemberId = store.currentMemberId;
  const member: Member | undefined = room?.members.find((m) => m.id === currentMemberId);

  // Not in room or session expired
  if (!room || !member) {
    return (
      <div style={{
        ...
        <h2 style={{ fontSize: 20, fontFamily: 'var(--font-display)', color: 'var(--text-primary)' }}>
          {!room ? '방을 찾을 수 없어요' : '참가 후 이용할 수 있어요'}
        </h2>
        <p style={{ fontSize: 14, color: 'var(--text-secondary)', textAlign: 'center' }}>
          {!room
            ? `코드 "${code}" 에 해당하는 방이 없어요`
            : '홈에서 방 코드를 입력해 참가해주세요'}
        </p>
