    ...
    (set, get) => ({
      ...
      subscribeRoom: (roomId: string) => {
        ...
        const handler = onValue(r, (snap) => {
          if (snap.exists()) {
            get()._setRoomData(snap.val() as Room);
          }
        });
      ...
      createRoom: async (name, maxMembers, profileImage) => {
        ...
        // Firebase에 방 저장 + code → roomId 인덱스 저장
        await dbSet(roomRef(room.id), room);
      ...
      joinRoom: async (code, memberName, avatar) => {
        ...
        // Firebase 업데이트 (members 배열만 업데이트)
        await update(roomRef(room.id), { members: updatedMembers });
