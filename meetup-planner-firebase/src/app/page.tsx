...
export default function Home() {
  ...
  const handleCreate = async () => {
    ...
    setLoading(true);
    try {
      const { room } = await store.createRoom(roomName.trim(), maxMembers, roomProfile || undefined);
      const result = await store.joinRoom(room.code, memberName.trim(), memberAvatar || undefined);
  ...
  const handleJoin = async () => {
    ...
    setLoading(true);
    try {
      // joinRoom이 이제 Firebase에서 방을 검색하므로 다른 기기에서도 동작
      const result = await store.joinRoom(joinCode.trim().toUpperCase(), memberName.trim(), memberAvatar || undefined);
